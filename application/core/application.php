...
class Application
{
    ...
    public function __construct()
    {
        ...
        // if no controller is given, index function of Results controller is called
        if (!$this->url_controller) {
            require_once APP . 'controllers/Results.php';
            $page = new Results();
            $page->index();

        } elseif (file_exists(APP . 'controllers/' . $this->url_controller . '.php')) {
            /* 
             * here we did check for controller: does such a controller file exist?
             * if so, then load this file and create this controller
             * example: if controller would be "User", then this line would translate into: $this->url_controller = new User();
             */
            ...
            if (method_exists($this->url_controller, $this->url_action)) {
                ...
                if(!empty($this->url_params)) {
                    ...
                    // If no parameters are given, just call the method without parameters, like $this->results->method();
                    $this->url_controller->{$this->url_action}();
                }

            }
            //if there is no such method call default method 'index'
            else {
            ...
            //if wrong controller name is given, show error page from Error controller
            require_once APP . 'controllers/error404.php';
            $page = new Error404();
            $page->index();
        }
